...
function Clients({ dataUser, dataClients }) {
  ...
  const permission = dataUser.roles.permissions.clients;
  ...
  const columns = [
    {
      ...
      cell: row => <div className="flex items-center gap-2">
        ...
        {
          can(permission, 'update') && <Link href={`/admin/clients/client/${row.id}`}>
    ...
    {
      name: 'Tél',
      selector: row => row.phone + (row.phone_second ? ' / ' + row.phone_second : ''),
      sortable: true,
  ...
  return (
    <>
      ...
      <div className="px-8 py-4 shadow-sm w-full relative">
        ...
        <TablesHeader to={can(permission, 'create') ? '/admin/clients/add' : null} title='Clients Liste' />
